...
declare -r FLOWERS_PROJECT=$(gcloud config list project --format "value(core.project)")
# View at https://console.cloud.google.com/dataflow
declare -r FLOWERS_JOB_ID="flowers_${USER}_$(date +%Y%m%d_%H%M%S)"
# View at https://console.cloud.google.com/storage
declare -r FLOWERS_BUCKET="gs://${FLOWERS_PROJECT}-dsikar"
# Location where pre-processing files will be stored 
declare -r FLOWERS_GCS_PATH="${FLOWERS_BUCKET}/${USER}/${FLOWERS_JOB_ID}"
declare -r FLOWERS_DICT_FILE=gs://cloud-samples-data/ml-engine/flowers/dict.txt
# Model name
declare -r FLOWERS_MODEL_NAME=flowers
declare -r FLOWERS_VERSION_NAME=v3
...
gcloud ml-engine jobs submit training "$FLOWERS_JOB_ID" \
  ...
  -- \
  --output_path "${FLOWERS_GCS_PATH}/training" \
...
gcloud ml-engine versions delete $FLOWERS_VERSION_NAME --model=$FLOWERS_MODEL_NAME -q --verbosity none
gcloud ml-engine models delete $FLOWERS_MODEL_NAME -q --verbosity none

# Tell CloudML about a new type of model coming.  Think of a "model" here as
# a namespace for deployed Tensorflow graphs.
gcloud ml-engine models create "$FLOWERS_MODEL_NAME" \
  --regions us-central1
...
gcloud ml-engine versions create "$FLOWERS_VERSION_NAME" \
  --model "$FLOWERS_MODEL_NAME" \
  --origin "${FLOWERS_GCS_PATH}/training/model" \
  --runtime-version=1.10
...
gcloud ml-engine versions set-default "$FLOWERS_VERSION_NAME" --model "$FLOWERS_MODEL_NAME"
...
sleep 10m
gcloud ml-engine predict --model ${FLOWERS_MODEL_NAME} --json-instances request.json
