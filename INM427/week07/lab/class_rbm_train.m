...
% Function to train the classification RBM given a configuration.
%
% This code demonstrates how the RBM can be used both as a feature extractor
% (generative/unsupervised learning) or a classifier (discriminative/supervised
% learning).
%
%   Input
%   -----
%   conf: Matlab structure
%     Configuration for the classification RBM.
%
%   Output
%   ------
...
% Decide whether or not to use GPU for training
...
% Load validation inputs and labels, and determine whether or not to use GPU
% for validation
...
% Load test inputs and labels, and determine whether or not to use GPU
% for testing  
...
hidNum = conf.hidNum;  % Number of hidden units
labNum = size(unique(trn_lab),1);  % Number of classes
bNum = conf.bNum;  % Number of batches
if conf.bNum == 0, bNum   = round(SZ/conf.sNum); end

% Initialise weight matrices. Here, W corresponds to the matrix between the
% hidden layers and inputs. U corresponds to that between hidden layer and the
% one-hot encodings of the labels
...
% Initialise visible, hidden and label unit biases
model.visB = 0.00*randn(1,visNum);
model.hidB = 0.00*randn(1,hidNum);
model.labB = 0.00*randn(1,labNum);

% Matrices to store derivatives of various parameters
...
% Check for GPU use once again as above for the data matrices
if gpu
...
% Initialise training hyperparameters and variables
lr = conf.params(1);  % Learning rate
tic;  % Start timer (timer ends with a "toc" at the bottom of the script)
vld_best = 0;  % Best validation cost
es_count = 0;  % Early-stopping counter 
acc_drop_count = 0;  % Number of consecutive iterations of drop in accuracy
vld_acc  = 0;  % Validation accuracy
tst_acc  = 0;  % Test accuracy
e = 0;  % Training epoch
running =1;  % Flag to terminate training
...
while running    
    e = e+1;
    inx = randperm(SZ); % Randomise training samples in batch
    %dat = dat(inx,:);
    %lab = lab(inx);
    ...
    for b=1:bNum  % Iterate over batches
        ...
        % Compute hidden-layer probabilities (activations)
...
%         hidP = logistic(visP*model.W + repmat(model.hidB,sNum,1));
        ...
        % A Hybrid-DRBM (HDRBM) is trained below which is jointly optimised
        % using generative and discriminative learning.
        if conf.gen>0  % Generative learning part
            %% Carry out block Gibbs sampling 'gNum' times
            for g=1:conf.gNum
    ...
    %        acc_e = acc_e + sum(sum(labP == labN))/sNum;
        ...
        if conf.dis>0  % Discriminative learning part 
    ...
    if exist('vld_lab','var')  % Compute validation accuracy
    ...
    if exist('tst_lab','var')  % Compute test accuracy
